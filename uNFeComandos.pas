...
function EnviarNFeSinc(AServidor, ACnpj: string; ANFe: TStream; AContingencia: Boolean; AidToken, ACSC: string; var ANroProtocolo: string): string;
...
procedure NFeIntegracaoContab(AServidor, ACnpj, ACNPJConsulta, AMod: string; AindXML, AindEmit, AindDest: Integer; AultNSU: Int64; AXml: TStream);
...
procedure MDFe_GerarPDF(AServidor, ACnpj, AMensagem, ARodape: string; AXml, ALogo, APdf: TStream);
...
procedure NFeIntegracaoContabToFiles(AXML, ADir: string);
...
//    FLabel: TLabel;
  ...
  private
    ...
    FMensagem: string;
  public
    constructor Create(AMensagem: string); reintroduce; virtual;
    procedure AfterConstruction; override;
    procedure BeforeDestruction; override;
  end;

function CriarTCPClient(AServidor, AMensagem: string): TIdTCPClient;
begin
  Result      := TTCPClient.Create(AMensagem);
...
begin
  ...
  TCPClient := CriarTCPClient(AServidor, 'Enviando nota... Aguarde...');
  try
    try
...
function EnviarNFeSinc(AServidor, ACnpj: string; ANFe: TStream; AContingencia: Boolean;
  AidToken, ACSC: string; var ANroProtocolo: string): string;
var
  TCPClient: TIdTCPClient;
  Codigo: Integer;
  sErro: string;
begin
  Result := EmptyStr;
  TCPClient := CriarTCPClient(AServidor, 'Enviando nota síncrona ... Aguarde...');
  try
    try
      TCPClient.Connect(1000);
      TCPClient.WriteLn(ACnpj);
      TCPClient.WriteInteger(Integer(NFe_EnviarNFSinc));
      TCPClient.WriteStream(ANFe, True, True);
      TCPClient.WriteInteger(Integer(AContingencia));
      TCPClient.WriteLn(AidToken);
      TCPClient.WriteLn(ACSC);
      ...
      begin
        Result := TCPClient.ReadLn;
        ANroProtocolo := TCPClient.ReadLn;
        TMemoryStream(ANFe).Clear;
        TCPClient.ReadStream(ANFe);
      end
      else if (Codigo = NFe_Error) then
      begin
        sErro := TCPClient.ReadLn;
        sErro := AnsiReplaceText(sErro, '##', sLineBreak);
        raise Exception.Create(sErro +#13+#13+#13+#13);
      end;
    finally
      TCPClient.Disconnect;
    end;
...
begin
  ...
  TCPClient := CriarTCPClient(AServidor, 'Buscando status da nota... Aguarde...');
  try
    try
      TCPClient.Connect(1000);
      TCPClient.WriteLn(ACnpj);
...
begin
  TCPClient := CriarTCPClient(AServidor, 'Cancelando nota... Aguarde...');
  try
...
begin
  Result := False;
  TCPClient := CriarTCPClient(AServidor, 'Verificando status da nota... Aguarde...');
  try
    try
      TCPClient.Connect(1000);
      TCPClient.WriteLn(ACNPJ);
...
begin
  Result := False;
  TCPClient := CriarTCPClient(AServidor, 'Verificando se conectado na internet... Aguarde...');
  try
    try
      TCPClient.Connect(1000);
      TCPClient.WriteLn(ACNPJ);
...
begin
  ...
  TCPClient := CriarTCPClient(AServidor, 'Gerando código de barras... Aguarde...');
  try
    Dados := TStringList.Create;
    DadosStream := TMemoryStream.Create;
    try
...
var
  ...
  DadosStream: TMemoryStream;
begin
  TCPClient := CriarTCPClient(AServidor, 'Criando chave nfe... Aguarde...');
  try
    Dados := TStringList.Create;
    DadosStream := TMemoryStream.Create;
    try
...
var
  ...
  Codigo: Integer;
begin
  TCPClient := CriarTCPClient(AServidor, 'Consultando nota... Aguarde...');
  try
    try
      TCPClient.Connect(1000);
      TCPClient.WriteLn(ACnpj);
...
var
  ...
  Codigo: Integer;
begin
  TCPClient := CriarTCPClient(AServidor, 'Adicionando protocolo da nota... Aguarde...');
  try
    try
      TCPClient.Connect(1000);
      TCPClient.WriteLn(ACnpj);
...
var
  ...
  DadosStream: TMemoryStream;
begin
  TCPClient := CriarTCPClient(AServidor, 'Inutilizando nota... Aguarde...');
  try
    Dados := TStringList.Create;
    DadosStream := TMemoryStream.Create;
    try
...
begin
  ...
  ANFe.Position := 0;
  TCPClient := CriarTCPClient(AServidor, 'Gerando DANFE... Aguarde...');
  try
    try
      TCPClient.Connect(1000);
      TCPClient.WriteLn(ACnpj);
...
var
  ...
  Codigo: Integer;
begin
  TCPClient := CriarTCPClient(AServidor, 'Enviando carta de correção... Aguarde...');
  try
    try
      TCPClient.Connect(1000);
      TCPClient.WriteLn(ACnpj);
...
begin
  ...
  try
    ...
    AXMLStream.Position := 0;
    Strings.LoadFromStream(AXMLStream);
    Strings.Text := UTF8Encode(Strings.Text);
    Strings.SaveToFile(AArquivo);
  finally
    Strings.Free;
  end;
end;
...
var
  ...
  Arquivo: TFileStream;
begin
  TCPClient := CriarTCPClient(AServidor, 'Enviando email... Aguarde...');
  try
    try
      TCPClient.Connect(1000);
      TCPClient.WriteLn(ACnpj);
...
var
  ...
  Codigo: Integer;
begin
  TCPClient := CriarTCPClient(AServidor, 'Importando nota... Aguarde...');
  try
    try
      TCPClient.Connect(1000);
      TCPClient.WriteLn(ACnpj);
...
var
  ...
  Codigo: Integer;
begin
  TCPClient := CriarTCPClient(AServidor, 'Consultando nota pelo nsu... Aguarde...');
  try
    try
      TCPClient.Connect(1000);
      TCPClient.WriteLn(ACnpj);
...
var
  ...
  Codigo: Integer;
begin
  TCPClient := CriarTCPClient(AServidor, 'Consultando notas... Aguarde...');
  try
    try
      TCPClient.Connect(1000);
      TCPClient.WriteLn(ACnpj);
      TCPClient.WriteInteger(Integer(uNFeConsts.NFe_IntegracaoContab));
      TCPClient.WriteLn(ACNPJConsulta);
      TCPClient.WriteLn(AMod);
      TCPClient.WriteInteger(AindXML);
      TCPClient.WriteInteger(AindEmit);
      TCPClient.WriteInteger(AindDest);
      TCPClient.WriteLn(IntToStr(AultNSU));
...
var
  ...
  Codigo: Integer;
begin
  TCPClient := CriarTCPClient(AServidor, 'Enviando manifestação... Aguarde...');
  try
    try
      TCPClient.Connect(1000);
      TCPClient.WriteLn(ACnpj);
...
var
  ...
  Codigo: Integer;
begin
  TCPClient := CriarTCPClient(AServidor, 'Consultando cadastro contribuintes... Aguarde...');
  try
    try
      TCPClient.Connect(1000);
      TCPClient.WriteLn(ACnpj);
...
var
  ...
  Codigo: Integer;
begin
  TCPClient := CriarTCPClient(AServidor, 'Baixando nota... Aguarde...');
  try
    try
      TCPClient.Connect(1000);
      TCPClient.WriteLn(ACnpj);
...
begin
  ...
  TCPClient := CriarTCPClient(AServidor, 'Gerando QRCode... Aguarde...');
  try
    try
      TCPClient.Connect(1000);
      TCPClient.WriteLn(ACnpj);
...
//  FLabel.AutoSize := True;
  ...
  FPanel := TNxPanel.Create(FTela);
  FPanel.Name := 'pnlMensagem';
  FPanel.Parent := FTela;
  FPanel.Left := 0;
  FPanel.Top := 0;
  FPanel.Width := 895;
  ...
  FPanel.Caption := FMensagem;
  FPanel.UseDockManager := False;
  FPanel.ParentBackground := False;
  FPanel.ParentFont := False;
  FPanel.TabOrder := 0;
  FPanel.Font.Color := clBlue;
  FPanel.Font.Size := 20;
  FPanel.Font.Name := 'Verdana';
  ...
  //FTela.AutoSize    := True;
  FTela.ClientHeight := 73;
  FTela.ClientWidth := 895;
...
begin
  inherited;
  FAguarde := TAguarde.Mostrar(FMensagem);
//  FAguarde := TAguarde.Mostrar('Conectando com o servidor... Aguarde...');
end;
...
var
  ...
  Codigo: Integer;
begin
  TCPClient := CriarTCPClient(AServidor, 'Gerando lote... Aguarde...');
  try
    try
      TCPClient.Connect(1000);
      TCPClient.WriteLn(ACnpj);
...
var
  ...
  Codigo: Integer;
begin
  TCPClient := CriarTCPClient(AServidor, 'Consultando lote... Aguarde...');
  try
    try
      TCPClient.Connect(1000);
      TCPClient.WriteLn(ACnpj);
...
var
  ...
  Codigo: Integer;
begin
  TCPClient := CriarTCPClient(AServidor, 'Obtendo nota... Aguarde...');
  try
    try
      TCPClient.Connect(1000);
      TCPClient.WriteLn(ACnpj);
...
constructor TTCPClient.Create(AMensagem: string);
begin
  FMensagem := AMensagem;
...
var
  ...
  Codigo: Integer;
begin
  TCPClient := CriarTCPClient(AServidor, 'Consultando status nota... Aguarde...');
  try
    try
      TCPClient.Connect(1000);
      TCPClient.WriteLn(ACnpj);
...
var
  ...
  Codigo: Integer;
begin
  TCPClient := CriarTCPClient(AServidor, 'Criando chave da nota... Aguarde...');
  try
    try
      TCPClient.Connect(1000);
      TCPClient.WriteLn(ACnpj);
...
begin
  ...
  TCPClient := CriarTCPClient(AServidor, 'Consultando nota não encerrada... Aguarde...');
  try
    try
      TCPClient.Connect(1000);
      TCPClient.WriteLn(ACnpj);
...
var
  ...
  Codigo: Integer;
begin
  TCPClient := CriarTCPClient(AServidor, 'Encerrar nota... Aguarde...');
  try
    try
      TCPClient.Connect(1000);
      TCPClient.WriteLn(ACnpj);
...
var
  ...
  Codigo: Integer;
begin
  TCPClient := CriarTCPClient(AServidor, 'Incluindo condutor... Aguarde...');
  try
    try
      TCPClient.Connect(1000);
      TCPClient.WriteLn(ACnpj);
...
var
  ...
  Codigo: Integer;
begin
  TCPClient := CriarTCPClient(AServidor, 'Consultar distribuição... Aguarde...');
  try
    try
      TCPClient.Connect(1000);
      TCPClient.WriteLn(ACnpj);
...
var
  ...
  Codigo: Integer;
begin
  TCPClient := CriarTCPClient(AServidor, 'Consultando distribuição... Aguarde...');
  try
    try
      TCPClient.Connect(1000);
      TCPClient.WriteLn(ACnpj);
...
begin
  ...
  try
    try
      ...
      TCPClient.WriteStream(AXml, True, True);
      TCPClient.WriteStream(ALogo, True, True);

      Codigo := TCPClient.ReadInteger;
      if (Codigo = NFe_OK) then
      begin
        TMemoryStream(APdf).Clear;
        TCPClient.ReadStream(APdf);
      end
      else
        raise EExceptionMDFe.Create( AnsiReplaceText(TCPClient.ReadLn, #8, sLineBreak) );
    finally
...
begin
  lStr := TStringStream.Create(aText);
  Mem := TMemoryStream.Create;
  try
    Mem.LoadFromStream(lStr);
    Mem.Position := 0;
    lDS := TZDecompressionStream.Create(Mem, 15+16);
    try
...
begin
  ...
  xDoc := LoadXMLData(AXML);
  xDoc.Options := xDoc.Options + [doNodeAutoIndent];
  xLista := xDoc.ChildNodes.FindNode('loteDistNFeRS');
  if (xLista <> nil) then
  begin
    for I := 0 to xLista.ChildNodes.Count - 1 do
    begin
      xProc := xLista.ChildNodes[I];
      xNode := xProc.ChildNodes[0];

      sName := IncludeTrailingPathDelimiter(ADir) +
               xNode.NodeName + '_' +
               xProc.Attributes['chAcesso'] +
               '.xml';

      sStream := TStringStream.Create('<?xml version="1.0" encoding="utf-8"?>' + xNode.XML);
      try
        SalvarUTF8(sStream, sName);
      finally
        FreeAndNil(sStream);
      end;
    end;
